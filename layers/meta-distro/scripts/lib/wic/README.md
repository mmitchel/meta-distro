...
This directory contains WIC layout templates (`.wks.in`). LVM layouts (`lvm-boot*`) use the custom `lvmrootfs` plugin, while `lvm-simple.wks.in` uses the standard Poky `rootfs` source.

## Plugin: lvmrootfs

The `lvmrootfs` plugin creates a partition containing an LVM physical volume with:
- A rootfs logical volume
- Optional additional logical volumes as specified

### Key Features

- **Direct system calls**: Uses Python subprocess to call system binaries directly (no sudo wrapper)
- **No HOSTTOOLS needed**: Works with standard Linux tools available on the build host
- **Simple architecture**: Direct calls to udisksctl, LVM, cryptsetup, mount utilities
...
- Direct calls to udisksctl for loop devices
...
- Direct calls to udisksctl for mount/unmount operations
...
All canned WKS files use the `.wks.in` template format. Placeholders are substituted using values from `layers/meta-distro/conf/distro/include/defaults.inc`.
...
- `@UUID_ESP@` → Filesystem UUID for ESP
- `@UUID_XBOOTLDR@` → Filesystem UUID for XBOOTLDR
- `@UUID_ROOT@` → Filesystem UUID for root LV
- `@UUID_VAR@` → Filesystem UUID for /var LV
...
#### lvm-boot.wks.in (Full Layout)
```
# EFI System Partition for systemd-boot
# Partition Type: @PARTTYPE_ESP@ (ESP)
...
# LUKS-encrypted LVM partition with rootfs + additional volumes
...
bootloader --ptable gpt --timeout=5 \
    --append="root=UUID=@UUID_ROOT@ rootflags=ro rootfstype=ext4 systemd.unified_cgroup_hierarchy=1 cgroup_no_v1=all"
```

#### lvm-simple.wks.in (Rootfs Only)
```
# EFI System Partition for systemd-boot
# Partition Type: @PARTTYPE_ESP@ (ESP)
part /boot/efi --source bootimg-efi --sourceparams="loader=systemd-boot" \
...
# Root partition (LUKS encrypted)
# Partition Type: @PARTTYPE_ROOT@ (SD_GPT_ROOT_X86_64)
part / --source rootfs --ondisk sda --fstype=ext4 --label rootfs \
    --align 1024 --size 4096M \
    --uuid=@UUID_ROOT@ --part-type=@PARTTYPE_ROOT@
...
%<buildgroup> ALL=(ALL) NOPASSWD: /usr/bin/udisksctl, /sbin/lvm, /bin/mount, /bin/umount, /sbin/cryptsetup
```
...
2. **Setup loop device**: Attach sparse file via udisksctl loop-setup
...
### Error: "Tool 'udisksctl' not found"

**Cause**: LVM tools not installed or not in PATH

**Solution**:
- Install lvm2: `sudo apt-get install lvm2` (Ubuntu/Debian)
- Install cryptsetup: `sudo apt-get install cryptsetup`
- Verify tools are in PATH: `which udisksctl lvm mkfs.ext4`
